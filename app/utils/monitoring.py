...
class SystemMonitor:
    ...
    async def collect_and_store_metrics(self):
        ...
        if system_metrics:
            # Store key metrics
            metrics_collector.set_gauge('cpu_percent', system_metrics['cpu']['percent'], unit='%')
            metrics_collector.set_gauge('memory_percent', system_metrics['memory']['percent'], unit='%')
            metrics_collector.set_gauge('disk_percent', system_metrics['disk']['percent'], unit='%')
            metrics_collector.set_gauge('process_memory_mb', system_metrics['process']['memory_rss_mb'], unit='MB')
            metrics_collector.set_gauge('uptime_seconds', system_metrics['process']['uptime_seconds'], unit='s')
...
class PerformanceMonitor:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.auto_record:
            ...
            metrics_collector.record_performance(self.operation, duration_ms, self.success)
    ...
    def check_thresholds(self) -> List[Dict[str, Any]]:
        ...
        for operation, stats in performance_stats.items():
            ...
            if stats.avg_duration > self.thresholds['response_time_ms']:
                if self._should_send_alert(f'response_time_high_{operation}', current_time):
                    alerts.append({
            ...
            if stats.count > 0:
                ...
                if error_rate > self.thresholds['error_rate_percent']:
                    if self._should_send_alert(f'error_rate_high_{operation}', current_time):
                        alerts.append({
...
def get_metrics_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        'last_health_check': health_checker.last_check.isoformat() if health_checker.last_check else None,
...
def record_api_metrics(endpoint: str, method: str, status_code: int, duration_ms: float):
    ...
    if 200 <= status_code < 400:
        metrics_collector.increment_counter(f"api_requests_success", labels={'endpoint': endpoint})
    else:
        metrics_collector.increment_counter(f"api_requests_error", labels={'endpoint': endpoint, 'status': str(status_code)})


def record_llm_metrics(provider: str, model: str, tokens_used: int, duration_ms: float, success: bool):
    ...
    if success:
        metrics_collector.increment_counter("llm_requests_success", labels={'provider': provider, 'model': model})
    else:
        metrics_collector.increment_counter("llm_requests_error", labels={'provider': provider, 'model': model})


def record_database_metrics(operation: str, table: str, duration_ms: float, success: bool):
    ...
    if success:
        metrics_collector.increment_counter("db_operations_success", labels={'operation': operation, 'table': table})
    else:
        metrics_collector.increment_counter("db_operations_error", labels={'operation': operation, 'table': table})
