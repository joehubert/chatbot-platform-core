...
class ModelConfig(BaseModel):
    ...
    max_tokens: Optional[int] = Field(None, ge=1, le=32000, description="Maximum tokens per request")
    ...
    cost_per_1k_tokens: Optional[float] = Field(None, ge=0.0, description="Cost per 1000 tokens")
    enabled: bool = Field(default=True, description="Whether model is enabled")
    fallback_models: List[str] = Field(default_factory=list, description="Fallback model identifiers")
...
class RateLimitConfig(BaseModel):
    ...
    per_user_per_minute: int = Field(default=60, ge=1, le=1000, description="Requests per user per minute")
    global_per_minute: int = Field(default=1000, ge=1, le=10000, description="Global requests per minute")
    burst_capacity: int = Field(default=10, ge=1, le=100, description="Burst capacity for token bucket")
    rate_limit_window_seconds: int = Field(default=60, ge=1, le=3600, description="Rate limit window in seconds")
    cleanup_interval_seconds: int = Field(default=300, ge=60, le=3600, description="Cleanup interval for expired tokens")
    block_duration_minutes: int = Field(default=5, ge=1, le=60, description="Block duration when limits exceeded")
...
class CacheConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Whether semantic caching is enabled")
    strategy: CacheStrategy = Field(default=CacheStrategy.VECTOR_SIMILARITY, description="Cache strategy")
    similarity_threshold: float = Field(default=0.85, ge=0.0, le=1.0, description="Similarity threshold for cache hits")
    ttl_hours: int = Field(default=24, ge=1, le=8760, description="Cache TTL in hours")
    max_cache_size: int = Field(default=10000, ge=100, le=100000, description="Maximum cache entries")
    cleanup_interval_hours: int = Field(default=6, ge=1, le=24, description="Cache cleanup interval in hours")
...
class AuthConfig(BaseModel):
    ...
    otp_expiry_minutes: int = Field(default=5, ge=1, le=30, description="OTP expiry time in minutes")
    max_auth_attempts: int = Field(default=3, ge=1, le=10, description="Maximum authentication attempts")
    lockout_duration_minutes: int = Field(default=15, ge=5, le=120, description="Lockout duration after max attempts")
    sms_provider: Optional[str] = Field(None, description="SMS provider (twilio, aws_sns)")
    email_provider: Optional[str] = Field(None, description="Email provider (sendgrid, aws_ses)")
...
class VectorDBConfig(BaseModel):
    ...
    index_name: str = Field(default="chatbot_knowledge", description="Index/collection name")
    dimension: int = Field(default=1536, ge=128, le=4096, description="Vector dimension")
    similarity_metric: str = Field(default="cosine", description="Similarity metric")
    top_k: int = Field(default=5, ge=1, le=50, description="Number of similar vectors to retrieve")
...
class DocumentConfig(BaseModel):
    ...
    chunk_size: int = Field(default=1000, ge=100, le=8000, description="Text chunk size for processing")
    chunk_overlap: int = Field(default=200, ge=0, le=1000, description="Overlap between chunks")
    auto_expire_days: Optional[int] = Field(None, ge=1, le=3650, description="Auto-expire documents after days")
...
class SystemConfig(BaseModel):
    ...
    models: Dict[str, ModelConfig] = Field(..., description="Model configurations by identifier")
    rate_limiting: RateLimitConfig = Field(default_factory=RateLimitConfig, description="Rate limiting settings")
    ...
    documents: DocumentConfig = Field(default_factory=DocumentConfig, description="Document processing settings")
    ...
    def validate_models(cls, v):
        ...
        required_roles = {ModelRole.RELEVANCE_CHECK, ModelRole.SIMPLE_QUERY, ModelRole.COMPLEX_QUERY}
...
class ConfigUpdate(BaseModel):
    ...
    models: Optional[Dict[str, ModelConfig]] = Field(None, description="Model configurations to update")
    rate_limiting: Optional[RateLimitConfig] = Field(None, description="Rate limiting settings")
    ...
    vector_db: Optional[VectorDBConfig] = Field(None, description="Vector database settings")
    documents: Optional[DocumentConfig] = Field(None, description="Document processing settings")
    fallback_error_message: Optional[str] = Field(None, description="Fallback error message")
...
class ConfigValidationResult(BaseModel):
    ...
    missing_env_vars: List[str] = Field(default_factory=list, description="Missing environment variables")
    
    class Config:
        schema_extra = {
            "example": {
                "valid": False,
                "errors": ["Model 'gpt-4' requires OPENAI_API_KEY environment variable"],
...
class MetricsData(BaseModel):
    ...
    requests_per_minute: int = Field(..., ge=0, description="Current requests per minute")
