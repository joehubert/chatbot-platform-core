...
class ChatContext(BaseModel):
    ...
    page_url: Optional[str] = Field(
        None, description="URL of the page where chat was initiated"
    )
    user_agent: Optional[str] = Field(
        None, description="User agent string from browser"
    )
...
class ChatMessage(BaseModel):
    ...
    message: str = Field(
        ..., min_length=1, max_length=4000, description="User message content"
    )
    ...
    context: Optional[ChatContext] = Field(
        default_factory=ChatContext, description="Context information"
    )
...
class ChatResponse(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Response confidence score"
    )
...
class MessageHistory(BaseModel):
    ...
    processing_time_ms: Optional[int] = Field(
        None, description="Processing time for assistant messages"
    )
...
class ConversationHistory(BaseModel):
    ...
    user_identifier: Optional[str] = Field(
        None, description="User identifier if available"
    )
    ...
    resolved: bool = Field(
        default=False, description="Whether conversation was resolved"
    )
    resolution_attempts: int = Field(
        default=0, ge=0, description="Number of resolution attempts"
    )
    authenticated: bool = Field(
        default=False, description="Whether user was authenticated"
    )
...
class ConversationSummary(BaseModel):
    ...
    user_satisfaction: Optional[float] = Field(
        None, ge=0.0, le=5.0, description="User satisfaction score"
    )
    duration_minutes: Optional[int] = Field(
        None, ge=0, description="Conversation duration in minutes"
    )
