...
class DocumentUpload(BaseModel):
    ...
    title: Optional[str] = Field(None, max_length=500, description="Document title override")
    description: Optional[str] = Field(None, max_length=2000, description="Document description")
    tags: List[str] = Field(default_factory=list, description="Tags for document organization")
    ...
    def validate_filename(cls, v):
        ...
        if extension not in allowed_extensions:
            raise ValueError(f'File type .{extension} not supported. Allowed: {allowed_extensions}')
...
class DocumentStatus(BaseModel):
    ...
    status: ProcessingStatus = Field(..., description="Current processing status")
    progress_percentage: Optional[float] = Field(None, ge=0.0, le=100.0, description="Processing progress")
...
class DocumentSearch(BaseModel):
    ...
    status_filter: Optional[List[ProcessingStatus]] = Field(None, description="Filter by status")
...
class DocumentSearchResult(BaseModel):
    ...
    category: DocumentCategory = Field(..., description="Document category")
    tags: List[str] = Field(default_factory=list, description="Document tags")
    relevance_score: float = Field(..., ge=0.0, le=1.0, description="Relevance score for query")
    matched_chunks: List[str] = Field(default_factory=list, description="Relevant text chunks")
...
class BulkDocumentOperation(BaseModel):
    ...
    document_ids: List[UUID] = Field(..., min_items=1, max_items=100, description="List of document IDs")
    operation: str = Field(..., description="Operation to perform")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Operation parameters")
    
    @validator('operation')
    def validate_operation(cls, v):
        allowed_operations = {'delete', 'reprocess', 'update_category', 'update_tags', 'extend_expiration'}
...
class BulkOperationResponse(BaseModel):
    ...
    successful: int = Field(default=0, ge=0, description="Successfully processed documents")
