...
class AuthRequest(BaseModel):
    ...
    contact_method: str = Field(
        ..., description="Method for token delivery: 'sms' or 'email'"
    )
    contact_value: str = Field(
        ..., description="Phone number or email address for token delivery"
    )
...
class AuthVerification(BaseModel):
    ...
    token: str = Field(
        ..., min_length=4, max_length=10, description="Authentication token received"
    )
    ...
    class Config:
        schema_extra = {"example": {"session_id": "session_12345", "token": "ABC123"}}
...
class AuthResponse(BaseModel):
    ...
    success: bool = Field(
        ..., description="Whether authentication request was successful"
    )
    ...
    retry_after: Optional[int] = Field(
        None, description="Seconds before retry is allowed"
    )
...
class TokenResponse(BaseModel):
    ...
    session_token: Optional[str] = Field(
        None, description="Session token for authenticated requests"
    )
    expires_at: Optional[datetime] = Field(
        None, description="Session token expiration timestamp"
    )
...
class UserProfile(BaseModel):
    ...
    authentication_count: int = Field(
        default=0, ge=0, description="Total number of authentications"
    )
...
class AuthStatus(BaseModel):
    ...
    authenticated: bool = Field(..., description="Whether session is authenticated")
    expires_at: Optional[datetime] = Field(
        None, description="Authentication expiration"
    )
    time_remaining: Optional[int] = Field(None, description="Seconds until expiration")
    requires_renewal: bool = Field(
        default=False, description="Whether authentication needs renewal"
    )
