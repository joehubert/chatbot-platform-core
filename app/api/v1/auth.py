...
async def request_auth_token(
    ...
    try:
        ...
        logger.info(f"OTP requested for session {request.session_id} via {request.contact_method}")
        ...
        raise HTTPException(
            status_code=500,
            detail="Error sending authentication code"
        )
...
async def verify_auth_token(
    ...
    try:
        ...
        if not verification_result.valid:
            raise HTTPException(
                status_code=400,
                detail=verification_result.error_message or "Invalid or expired authentication code"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Error verifying authentication code"
        )
...
async def get_session_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error checking session status"
        )
...
async def logout_session(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error logging out session"
        )
...
async def extend_session(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error extending session"
        )
