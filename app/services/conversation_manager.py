...
class ConversationManager:
    ...
    async def get_conversation(
        ...
        try:
            ...
            if conversation:
                ...
                if datetime.utcnow() - conversation.started_at > self.max_conversation_age:
    ...
    async def increment_resolution_attempts(self, conversation_id: UUID) -> None:
        ...
        try:
            ...
            if conversation:
                ...
                logger.debug(f"Incremented resolution attempts for conversation {conversation_id}")
    ...
    async def mark_conversation_resolved(
        ...
        try:
            ...
            if conversation:
                ...
                logger.info(f"Marked conversation {conversation_id} as {'resolved' if resolved else 'unresolved'}")
    ...
    async def get_conversation_summary(self, conversation_id: UUID) -> ConversationSummary:
        ...
        try:
            ...
            return ConversationSummary(
                ...
                avg_processing_time_ms=sum(m.processing_time_ms for m in messages) / len(messages) if messages else 0
            )
    ...
    async def get_conversation_metrics(
        ...
        try:
            ...
            avg_resolution_attempts = self.db.query(
                func.avg(Conversation.resolution_attempts)
            ).filter(
                and_(
                    Conversation.started_at >= start_date,
                    Conversation.started_at <= end_date
            ...
            message_stats = self.db.query(
                ...
                func.count(Message.id).filter(Message.cached == True).label('cached_messages')
            ...
            cache_hit_rate = (cached_messages / total_messages) if total_messages > 0 else 0
    ...
    async def get_conversation_history(
        ...
        """
        try:
            conversation = await self.get_conversation(session_id, create_if_not_exists=False)
